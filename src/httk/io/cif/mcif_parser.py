...
def _parse_moments(block, *, k_sigma=2.0, equalize=True, resolution=True):
    """
    ...
    for i in range(n):
        ...
        if equalize and i < len(forms):
            ...
            if (
                ...
                and m_esd is not None and m_esd > 0.0
            ):
                sigma_comp = m_esd / (3.0 ** 0.5)
                mean_comp = (mx + my + mz) / 3.0
                if max(abs(mx - mean_comp),
                       abs(my - mean_comp),
                       abs(mz - mean_comp)) <= k_sigma * sigma_comp:
                    mx = my = mz = mean_comp
    ...
    for m in re.finditer(token_re, s):
        ...
        if var is not None:
            if var not in allowed_vars:
                raise ValueError(
                    f"Expression '{expr}' references {var}, not in allowed {allowed_vars}."
                )
            ...
            if coef_str in (None, ""):
                ...
                if f.denominator != 1:
                    raise ValueError(
                        f"Non-integer coefficient {coef_str} on {var} in '{expr}'"
                    )
...
def _parse_mag_asu_cell(cifblock, *, moment_equalization=True):
    ...
    cif_moments, momlabels, spin_basis, magres = _parse_moments(cifblock,equalize=moment_equalization, resolution=True)

    if spin_basis == "crystal":
        cif_moments = _crystal_to_cartesian(cif_moments, basis)
        spin_basis = "cartesian"
...
def cifblock_to_mag_asu(cifblock, *, error_on_nonmag=False):
    ...
    if base_symops_xyz is None:
        ...
        base_symops = xyz_symops_to_matrix(
            base_symops_xyz, use_fractions=True, time_reversal_convention="spglib"
        )
    ...
    if centering_symops_xyz is None:
        ...
        if centering_symops_alg is None:
            ...
            cent_symops = xyz_symops_to_matrix(
    ...
    result = {'basis':basis, 'positions':positions, 'symbols':symbols, 'symops':symops, 'incomm':incomm, 'space_group_nbr':space_group_nbr, 'space_group_name_hm':space_group_name_hm, 'icsd':icsd, 'doi':doi, 'spin_basis':spin_basis, 'magmoms':magmoms, 'bns_nbr':bns_nbr, 'bns_name':bns_name, 'equivalent_atoms':equivalent_atoms, 'resolution':res, 'magmom_resolution':magres, 'labels':labels}
