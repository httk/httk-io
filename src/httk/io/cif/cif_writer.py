...
def _cif_validate_name(name_unfiltered, context=None):
    ...
    if len(name) > 75:
        sys.stderr.write(
            "***Warning: write_cif: name length > 75, surplus characters removed in "
            + str(context)
            + ": "
            + name_unfiltered
        )
...
def _cif_write_semicolontextfield(f, lines, noteol, max_line_length):
    ...
    for i in range(len(lines)):
        ...
        if lines[i][0] == ';':
            sys.stderr.write(
                "***Warning: write_cif: had to insert space before semicolon at the start of a line of a multi-line string to fulfill arcane quoting rules."
            )
            lines[i] = ' ' + lines[i]
            ...
            for i in range(1, len(sublines)):
                if sublines[i][0] == ";":
                    if len(sublines[i]) > 1 and sublines[i][1] != ";":
                        ...
                        sys.stderr.write(
                            "***Warning: write_cif: had to insert space before semicolon in a long string to fulfill arcane quoting rules."
                        )
                ...
                for i in range(len(outdata_columns[0])):
                    ...
                    for j in range(len(outdata_columns)):
                        ...
                        if column_count > max_line_length and noteol:
                            ...
                            column_count = 0
                            noteol = False
                        noteol = _cif_write_data_value(
                            f, outdata_columns[j][i], noteol, max_line_length, use_types, inloop=True
                        )
                        if noteol:
                            f.write(" ")
                            column_count += 1
