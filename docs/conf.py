...
# httk-io registers its cif loader against httk-core's plugin registry and imports
# httk.core, so cross-project references resolve against the published httk
# documentation site. The base URL comes from the DOCS_BASE_URL Makefile variable
# (exported as HTTK_DOCS_BASE_URL); the default below keeps bare sphinx invocations
# working. Inventories are vendored in docs/_inventories/ so docs builds need no
# network access; link targets still point at the live sites. Refresh the committed
...
# httk-io shares the PEP 420 "httk" namespace with httk-core but ships in a
# separate distribution: it imports httk.core (to register its loader) and its
# handler package resolves httk.core.register at runtime. AutoAPI's static parser
# cannot follow imports into that other distribution, so the
# "autoapi.python_import_resolution" notice is suppressed while all reference
# checking stays strict. Real cross-project references resolve via the httk-core
# intersphinx inventory above.
